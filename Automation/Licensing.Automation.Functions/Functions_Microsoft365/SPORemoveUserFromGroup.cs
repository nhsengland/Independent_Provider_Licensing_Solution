...
{
    public record SPORemoveUserFromGroupData(string SiteUrl, string Group, string UserId);

    public class SPORemoveUserFromGroup
    {
        private readonly ILogger _logger;
        private readonly IPnPContextFactory _contextFactory;
        private readonly AzureFunctionSettings _azureFunctionSettings;
        
        public SPORemoveUserFromGroup(IPnPContextFactory pnpContextFactory, ILoggerFactory loggerFactory, AzureFunctionSettings azureFunctionSettings)
        {
            _logger = loggerFactory.CreateLogger<SPORemoveUserFromGroup>();
            _azureFunctionSettings = azureFunctionSettings;
            _contextFactory = pnpContextFactory;
        }

        [Function("SPORemoveUserFromGroup")]
        public async Task<HttpResponseData> RunAsync([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequestData req, [FromBody] SPORemoveUserFromGroupData data)
        {
            try
            {
                using (var pnpContext = await _contextFactory.CreateAsync(new Uri(data.SiteUrl)))
                {
                    ...
                    // get user
                    var spoUser = await group.Users.FirstOrDefaultAsync(u => string.Equals(u.UserPrincipalName, graphUser.UserPrincipalName, StringComparison.OrdinalIgnoreCase));
